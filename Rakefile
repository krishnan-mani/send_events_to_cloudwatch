...
task :put_log_events, [:log_line, :pattern, :starting_minutes_ago, :min_value, :max_value, :number_of_items, :log_group_name, :region] do |t, args|
  log_line = args.log_line || ENV['LOG_LINE']
  pattern = args.pattern || ENV['PATTERN']
  starting_minutes_ago = (args.pattern || ENV['STARTING_MINUTES_AGO']).to_i
  ...
  log_events_data = LogEventsGenerator.new(log_line, pattern).get_log_events(number_of_log_events, starting_minutes_ago, min_value, max_value)
